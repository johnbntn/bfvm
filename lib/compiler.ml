...
let compile in_file out_file out_file_type =
  ...
  | Object -> 
    ...
    Llvm_target.TargetMachine.emit_to_file the_module file_type out_file machine
  | Exe ->
    let file_type = Llvm_target.CodeGenFileType.ObjectFile in
    let obj = "temporary_object_file.o" in
    let _ = Llvm_target.TargetMachine.emit_to_file the_module file_type obj machine in
...
let run_compiler () = 
  let compiler_term = Term.(const compile $ input_file $ output_file $ output_file_type) in
  let compiler_cmd =
    let program = Cmd.info "bfc" ~doc:"A compiler for the Brainf*ck programming language" in
