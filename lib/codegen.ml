...
let build_ptr_op ~env name llvm_fn i32_type =
...
let build_tape_op ~env name llvm_fn i32_type i8_type i8_array_type =
  let load_ptr = Llvm.build_load i32_type env.ptr "load_ptr" env.builder in
  let tape_cell_addr =
    Llvm.build_gep i8_array_type env.tape
      [| Llvm.const_int i32_type 0; load_ptr |]
      "tape_cell_addr" env.builder
  in
  let tape_cell_value =
    Llvm.build_load i8_type tape_cell_addr "tape_cell_val" env.builder
  in
...
let codegen_op ~env token =
  ...
  match token with
  | T_INC -> build_ptr_op ~env "inc_ptr" Llvm.build_add i32_type
  | T_DEC -> build_ptr_op ~env "dec_ptr" Llvm.build_sub i32_type
  | T_PLUS ->
      build_tape_op ~env "add_to_cell" Llvm.build_add i32_type i8_type
        i8_array_type
  | T_MINUS ->
      build_tape_op ~env "sub_from_cell" Llvm.build_sub i32_type i8_type
        i8_array_type
  | T_LBRAC ->
      ...
      let new_loop =
        {
          header_bb =
            Llvm.append_block env.the_context
              ("loop_header_" ^ num_loops)
              env.main_fn;
          body_bb =
            Llvm.append_block env.the_context ("loop_body_" ^ num_loops)
              env.main_fn;
          after_bb =
            Llvm.append_block env.the_context
              ("loop_after_" ^ num_loops)
              env.main_fn;
        }
      in
      ...
      let cmp =
        Llvm.build_icmp Llvm.Icmp.Ne tape_cell_value const_zero "cmp"
          env.builder
      in
      let _ =
        Llvm.build_cond_br cmp new_loop.body_bb new_loop.after_bb env.builder
      in
      let _ = Llvm.position_at_end new_loop.body_bb env.builder in
      ()
  | T_RBRAC ->
      let _ =
        match Stack.pop env.loop_stack with
        | loop ->
  ...
  | T_COMMA ->
      ...
      let get_fn_call =
        Llvm.build_call get_fn_type get_fn [||] "get_char" env.builder
      in
      let trunc_get =
        Llvm.build_trunc get_fn_call i8_type "trunc_get" env.builder
      in
      let load_ptr = Llvm.build_load i32_type env.ptr "load_ptr" env.builder in
      let tape_cell_addr =
        Llvm.build_gep i8_array_type env.tape
  ...
  | T_DOT ->
      ...
      let _ =
        Llvm.build_call put_fn_type put_fn [| tape_cell_value |] "put_char"
          env.builder
      in
      ()
...
let generate tokens =
  ...
  let i8_array_type = Llvm.array_type i8_type 30000 in
  let tape =
    Llvm.define_global "tape" (Llvm.const_null i8_array_type) the_module
  in
  ...
  let i32_type = Llvm.i32_type the_context in
  let main_fn =
    Llvm.define_function "main" (Llvm.function_type i32_type [||]) the_module
  in
